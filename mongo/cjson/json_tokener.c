...
#define mac_obj_field_name tok->stack[tok->depth].obj_field_name
					  ...
					  const char *str, int len)
{
  ...
  redo_char:
	...
	while(1) {
	  if(c == tok->quote_char) {
	    printbuf_memappend_fast(tok->pb, case_start, str-case_start);
	    mac_obj_field_name = strdup(tok->pb->buf);
    ...
    case json_tokener_state_object_value_add:
      json_object_object_add(current, mac_obj_field_name, obj);
      free(mac_obj_field_name);
      mac_obj_field_name = NULL;
